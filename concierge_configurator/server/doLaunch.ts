...
export default async function* (
	options: FormData,
	state: { apiWorker?: Subprocess; webWorker?: Subprocess },
) {
	...
	if (environment == "local") {
		yield logMessage(
			"Building local code files to Docker image. This can take a while depending on your internet connection...",
		);
		await $`docker compose -f ./docker_compose/docker-compose-local.yml build`;
		yield logMessage(
			"Launching Docker Compose configuration with locally built image...",
		);
		await $`docker compose -f ./docker_compose/docker-compose-local.yml up -d`;
	} else if (environment == "development") {
		yield logMessage(
			"Launching Docker Compose configuration to run Shabti API code locally...",
		);
		await $`docker compose -f ./docker_compose/docker-compose-dev.yml up -d`;
		yield logMessage("Running Shabti from local codebase...");
		state.apiWorker = Bun.spawn(
			[
				platform() == "win32" ? ".\\Scripts\\python" : "./bin/python",
				"-m",
				"dev_launcher",
			],
			{
				cwd: path.resolve(
					path.join("..", "docker_containers", "concierge_api"),
				),
			},
		);
		...
		yield logMessage(
			"Launching Docker Compose configuration to run Shabti Web UI code locally...",
		);
		await $`docker compose -f ./docker_compose/docker-compose-dev.yml up -d`;
		yield logMessage("Running Shabti from local codebase...");
		state.webWorker = Bun.spawn(
			[
				platform() == "win32" ? ".\\Scripts\\python" : "./bin/python",
				"-m",
				"dev_launcher",
			...
			{
				cwd: path.resolve(
					path.join("..", "docker_containers", "concierge_web"),
				),
			},
		);
	} else {
