...
- Use the Keycloak administration UI to assign roles to users. Open up the Shabti realm and assign roles from the shabti-auth client.
...
def home_server(
    ...
    def profile():
        ...
        if "resource_access" in info:
            try:
                contents.append(
                    ui.p(
                        f"Roles: {', '.join(info['resource_access']['shabti-auth']['roles'])}"
                    )
                )
            except KeyError:
                pass
    ...
    def home_text():
        ...
        if info:
            try:
                has_admin = "admin" in info["resource_access"]["shabti-auth"]["roles"]
                if has_admin:
