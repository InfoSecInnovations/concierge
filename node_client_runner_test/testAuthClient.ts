...
const run = async () => {
  ...
  for await (const item of await client.insertUrls(collectionId, ["https://en.wikipedia.org/wiki/Generative_artificial_intelligence"])) {
    console.log(item)
  }
  await client.getDocuments(collectionId).then(documents => documents.forEach(documentInfo => console.log(documentInfo)))
  for await (const item of await client.prompt(collectionId, "Where does Generative AI get its data from?", "question")) {
    if (item.response) console.log(item.response)
    else console.log(item)
  }
  await client.deleteDocument(collectionId, "plaintext", documentId).then(documentId => console.log(documentId))
